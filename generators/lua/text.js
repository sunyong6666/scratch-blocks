...
Blockly.Lua['text_multiline'] = function(block) {
  ...
  const order =
      code.indexOf('..') !== -1 ? Blockly.Lua.ORDER_CONCATENATION : Blockly.Lua.ORDER_ATOMIC;
  return [code, order];
};
...
Blockly.Lua['text_join'] = function(block) {
  ...
  if (block.itemCount_ === 0) {
    ...
    const element = Blockly.Lua.valueToCode(block, 'ADD0', Blockly.Lua.ORDER_NONE) || "''";
    const code = 'tostring(' + element + ')';
    return [code, Blockly.Lua.ORDER_HIGH];
  } else if (block.itemCount_ === 2) {
    const element0 =
    Blockly.Lua.valueToCode(block, 'ADD0', Blockly.Lua.ORDER_CONCATENATION) || "''";
    const element1 =
    Blockly.Lua.valueToCode(block, 'ADD1', Blockly.Lua.ORDER_CONCATENATION) || "''";
...
Blockly.Lua['text_append'] = function(block) {
  ...
  const value =
  Blockly.Lua.valueToCode(block, 'TEXT', Blockly.Lua.ORDER_CONCATENATION) || "''";
  return varName + ' = ' + varName + ' .. ' + value + '\n';
};

Blockly.Lua['text_length'] = function(block) {
  // String or array length.
  const text = Blockly.Lua.valueToCode(block, 'VALUE', Blockly.Lua.ORDER_UNARY) || "''";
...
Blockly.Lua['text_indexOf'] = function(block) {
  ...
  const substring = Blockly.Lua.valueToCode(block, 'FIND', Blockly.Lua.ORDER_NONE) || "''";
  const text = Blockly.Lua.valueToCode(block, 'VALUE', Blockly.Lua.ORDER_NONE) || "''";
  let functionName;
  if (block.getFieldValue('END') === 'FIRST') {
    functionName = Blockly.Lua.provideFunction_('firstIndexOf', `
...
`);
    ...
    functionName = Blockly.Lua.provideFunction_('lastIndexOf', `
function ${Blockly.Lua.FUNCTION_NAME_PLACEHOLDER_}(str, substr)
...
Blockly.Lua['text_charAt'] = function(block) {
  ...
  const atOrder = (where === 'FROM_END') ? Blockly.Lua.ORDER_UNARY : Blockly.Lua.ORDER_NONE;
  const at = Blockly.Lua.valueToCode(block, 'AT', atOrder) || '1';
  const text = Blockly.Lua.valueToCode(block, 'VALUE', Blockly.Lua.ORDER_NONE) || "''";
  let code;
  if (where === 'RANDOM') {
    const functionName = Blockly.Lua.provideFunction_('text_random_letter', `
...
`);
    ...
    if (start.match(/^-?\w*$/)) {
      ...
      const functionName = Blockly.Lua.provideFunction_('text_char_at', `
...
Blockly.Lua['text_getSubstring'] = function(block) {
  ...
  const at1Order = (where1 === 'FROM_END') ? Blockly.Lua.ORDER_UNARY : Blockly.Lua.ORDER_NONE;
  ...
  const at2Order = (where2 === 'FROM_END') ? Blockly.Lua.ORDER_UNARY : Blockly.Lua.ORDER_NONE;
...
Blockly.Lua['text_changeCase'] = function(block) {
  ...
  const text = Blockly.Lua.valueToCode(block, 'TEXT', Blockly.Lua.ORDER_NONE) || "''";
  let functionName;
  if (operator === 'UPPERCASE') {
    ...
    functionName = Blockly.Lua.provideFunction_('text_titlecase', `
function ${Blockly.Lua.FUNCTION_NAME_PLACEHOLDER_}(str)
  local buf = {}
  local inWord = false
  for i = 1, #str do
...
Blockly.Lua['text_prompt_ext'] = function(block) {
  ...
  if (block.getField('TEXT')) {
    ...
    // External message.
    msg = Blockly.Lua.valueToCode(block, 'TEXT', Blockly.Lua.ORDER_NONE) || "''";
  }

  const functionName = Blockly.Lua.provideFunction_('text_prompt', `
...
Blockly.Lua['text_count'] = function(block) {
  const text = Blockly.Lua.valueToCode(block, 'TEXT', Blockly.Lua.ORDER_NONE) || "''";
  const sub = Blockly.Lua.valueToCode(block, 'SUB', Blockly.Lua.ORDER_NONE) || "''";
  const functionName = Blockly.Lua.provideFunction_('text_count', `
...
Blockly.Lua['text_replace'] = function(block) {
  const text = Blockly.Lua.valueToCode(block, 'TEXT', Blockly.Lua.ORDER_NONE) || "''";
  const from = Blockly.Lua.valueToCode(block, 'FROM', Blockly.Lua.ORDER_NONE) || "''";
  const to = Blockly.Lua.valueToCode(block, 'TO', Blockly.Lua.ORDER_NONE) || "''";
  const functionName = Blockly.Lua.provideFunction_('text_replace', `
