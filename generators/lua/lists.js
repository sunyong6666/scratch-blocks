...
Blockly.Lua['lists_repeat'] = function(block) {
  ...
  const functionName = Blockly.Lua.provideFunction_('create_list_repeated', `
function ${Blockly.Lua.FUNCTION_NAME_PLACEHOLDER_}(item, count)
  ...
  `);
  const element = Blockly.Lua.valueToCode(block, 'ITEM', Blockly.Lua.ORDER_NONE) || 'None';
  const repeatCount = Blockly.Lua.valueToCode(block, 'NUM', Blockly.Lua.ORDER_NONE) || '0';
...
Blockly.Lua['lists_length'] = function(block) {
  ...
  const list = Blockly.Lua.valueToCode(block, 'VALUE', Blockly.Lua.ORDER_UNARY) || '{}';
...
Blockly.Lua['lists_indexOf'] = function(block) {
  ...
  const item = Blockly.Lua.valueToCode(block, 'FIND', Blockly.Lua.ORDER_NONE) || "''";
  const list = Blockly.Lua.valueToCode(block, 'VALUE', Blockly.Lua.ORDER_NONE) || '{}';
  let functionName;
  if (block.getFieldValue('END') === 'FIRST') {
    functionName = Blockly.Lua.provideFunction_('first_index', `
...
`);
    ...
    functionName = Blockly.Lua.provideFunction_('last_index', `
function ${Blockly.Lua.FUNCTION_NAME_PLACEHOLDER_}(t, elem)
  for i = #t, 1, -1 do
    if t[i] == elem then
      return i
...
Blockly.Lua['lists_getIndex'] = function(block) {
  ...
  if ((where === 'LAST' || where === 'FROM_END' || where === 'RANDOM') &&
    ...
    if (mode === 'REMOVE') {
      ...
      const atOrder =
          (where === 'FROM_END') ? Blockly.Lua.ORDER_ADDITIVE : Blockly.Lua.ORDER_NONE;
      let at = Blockly.Lua.valueToCode(block, 'AT', atOrder) || '1';
      const listVar =
      Blockly.Lua.nameDB_.getDistinctName('tmp_list', Blockly.Names.VARIABLE);
      ...
      const at = Blockly.Lua.valueToCode(block, 'AT', Blockly.Lua.ORDER_NONE) || '1';
      let functionName;
      if (mode === 'GET') {
        functionName = Blockly.Lua.provideFunction_('list_get_' + where.toLowerCase(), [
...
Blockly.Lua['lists_setIndex'] = function(block) {
  ...
  let list = Blockly.Lua.valueToCode(block, 'LIST', Blockly.Lua.ORDER_HIGH) || '{}';
  const mode = block.getFieldValue('MODE') || 'SET';
  const where = block.getFieldValue('WHERE') || 'FROM_START';
  const at = Blockly.Lua.valueToCode(block, 'AT', Blockly.Lua.ORDER_ADDITIVE) || '1';
  const value = Blockly.Lua.valueToCode(block, 'TO', Blockly.Lua.ORDER_NONE) || 'None';
  ...
  if ((where === 'LAST' || where === 'FROM_END' || where === 'RANDOM') &&
    ...
    // `list` is an expression, so we may not evaluate it more than once.
    // We can use multiple statements.
    const listVar = Blockly.Lua.nameDB_.getDistinctName('tmp_list', Blockly.Names.VARIABLE);
    code = listVar + ' = ' + list + '\n';
    list = listVar;
  }
  if (mode === 'SET') {
...
Blockly.Lua['lists_getSublist'] = function(block) {
  ...
  const at1 = Blockly.Lua.valueToCode(block, 'AT1', Blockly.Lua.ORDER_NONE) || '1';
  const at2 = Blockly.Lua.valueToCode(block, 'AT2', Blockly.Lua.ORDER_NONE) || '1';
...
function ${Blockly.Lua.FUNCTION_NAME_PLACEHOLDER_}(source${at1Param}${at2Param})
  local t = {}
...
Blockly.Lua['lists_sort'] = function(block) {
  ...
  const functionName = Blockly.Lua.provideFunction_('list_sort', `
...
Blockly.Lua['lists_split'] = function(block) {
  ...
  let input = Blockly.Lua.valueToCode(block, 'INPUT', Blockly.Lua.ORDER_NONE);
  const delimiter = Blockly.Lua.valueToCode(block, 'DELIM', Blockly.Lua.ORDER_NONE) || "''";
...
Blockly.Lua['lists_reverse'] = function(block) {
  ...
  const list = Blockly.Lua.valueToCode(block, 'LIST', Blockly.Lua.ORDER_NONE) || '{}';
  const functionName = Blockly.Lua.provideFunction_('list_reverse', `
